    ...
    switch($_GET['action'])
    {
        case 'delete_file':
            if ($delete_files){
							...
							// additional by Willy ( delete from database )
							$last_slash_pos = strrpos($fullpath, "/");
							$filename = substr( $fullpath, ( $last_slash_pos + 1 ) );
							
							$con = mysqli_connect( "localhost", "root", "wil123", "kejorapro" );
							...
							else
							{
								...
								mysqli_query($con, "DELETE FROM medias WHERE url = '".rawurlencode($fullpath)."'");
								mysqli_close($con);
							}
							// END OF additional by Willy ( delete from database )
