...
class TemporarySendNewsletterCommand extends Command {
	 ...
	 * @var string
	 */
	protected $name = 'newsletter:temporarysendnewsletter';
	 ...
	 * @var string
	 */
	protected $description = 'Temporary Send Newsletter';
	...
	public function send( $id )
	{
		...
		echo "debug sebelum query data pake eloquance"."\r\n";
		echo time()."\r\n";
		...
		echo "debug setelah query data pake eloquance"."\r\n";
		echo time()."\r\n";
		...
		echo "debug SEBELUM foreach members"."\r\n";
		echo time()."\r\n";	
		...
		echo "MANDRILL MEMBERS : " . count( $arr_mandrill_members );
		echo '\r\n';
		echo "MAILGUN MEMBERS : " . count( $arr_mailgun_members );
		echo '\r\n';
		echo "SES MEMBERS : " . count( $arr_ses_members );
		echo '\r\n';
		...
		if ( count( $arr_mandrill_members ) > 0 )
		{
			...
			echo $mandrill_member_collections;
			echo '\r\n';
			...
			//Queue::push('SendMandrillEmail', array("newsletter_id" => $id, "member_collections" => $mandrill_member_collections));			
		}

		/* MAILGUN */
		/* -------------------------------------------- */
		// handle the newsletters to be sent by Mailgun
		if ( count( $arr_mailgun_members ) > 0 )
		{
			...
			// reason : the queue cannot accept BIG DATA so we handle the big data in its own controller
			//Queue::push('SendMailgunEmail', array("newsletter_id" => $id, "member_collections" => $mailgun_member_collections));
		}

		/* AMAZON SES */
		/* -------------------------------------------- */
		// handle the newsletters to be sent by Amazon SES
		if ( count( $arr_ses_members ) > 0 )
		{
			...
			// reason : the queue cannot accept BIG DATA so we handle the big data in its own controller
			//Queue::push('SendAWSEmail', array("newsletter_id" => $id, "member_collections" => $ses_member_collections));
		}
